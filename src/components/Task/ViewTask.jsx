...
export default function ViewTask() {
  ...
  const { modelStates, updateModelStates, taskData } =
    useContext(TaskModelStates)
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-2">
                      ...
                      <p className="text-sm text-gray-500">
                        {CONSTANTS_STRING.CREATED_AT}:{' '}
                        {moment(taskData?.createdAt).format('lll')}
                      </p>
                      <p className="text-sm text-gray-500">
                        {CONSTANTS_STRING.DUE_DATE}:{' '}
                        {moment(taskData?.dueDate).format('ll')}
                      </p>
                      {taskData?.isCompleted ? (
                        <p className="text-sm text-gray-500">
                          {CONSTANTS_STRING.COMPLETED_AT} :{' '}
                          {moment(taskData?.updatedAt).format('lll')}
                        </p>
                      ) : (
                        <></>
                      )}
