        ...
        <table className="min-w-full bg-white border border-gray-300 ">
          <thead>
            <tr className="text-left bg-gray-500 text-zinc-50">
              ...
              <th
                className="py-2 px-4 border-b inline-flex cursor-pointer"
                data-testid={"sort_tasks"}
                onClick={sortTasks}
              >
                <a>{CONSTANTS_STRING.DUE_DATE} </a>
                <TiArrowUnsorted />
              </th>
          ...
          <tbody className="text-left">
            {taskList?.map?.((task, index) => (
              <tr
                key={index}
                onClick={() => hanldeViewTask(task)}
                className="hover:bg-gray-50"
              >
                <td className="py-2 px-4 border-b font-bold">
                  {task?.title}
                  {task?.isCompleted ? (
                    <span className="bg-green-800 text-white text-xs font-medium h-6 p-1 ml-3 rounded dark:bg-red-900 dark:text-red-300">
                      Completed
                    </span>
                  ) : new Date() - new Date(task?.dueDate) > 0 ? (
                    <span className="bg-pink-700 text-white text-xs font-medium h-6 p-1 ml-3 rounded dark:bg-red-900 dark:text-red-300">
                      Over Due
                    </span>
                  ) : (
                    <span className="bg-blue-500 text-white text-xs font-medium h-6 p-1 ml-3 rounded dark:bg-red-900 dark:text-red-300">
                      Pending
                    </span>
                  )}
                </td>
